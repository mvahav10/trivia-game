...
useGLTF.preload('/shape_man_running.glb');  // Adjust this path to where your model is located

function Runner({ speed, isRunning }) {
  const group = useRef();
  const { scene, animations } = useGLTF('/shape_man_running.glb'); // Adjust this path as well
